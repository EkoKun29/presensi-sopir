...
<div id="content">
    ...
    <div class="container-fluid">
            ...
            <div class="d-sm-flex align-items-center justify-content-between mb-4">
                <h1 class="h3 mb-0 text-gray-800">Form Revisi Kop Sales Do</h1>
            </div>
                    ...
                    <div class="col-lg-6">
                        <div class="card shadow mb-6">
                            <div class="card-header py-3">
                                <h6 class="m-0 font-weight-bold text-primary">Form Revisi Kop Sales Do {{$detail->nomor_surat}} ({{date('Y-m-d', strtotime($detail->created_at))}})</h6>
                            </div>
                            <div class="card-body">
                                <form action="{{ route('do.updateKop', $detail->id) }}" method="POST">
                                @csrf
                                    ...
                                    <div class="card-body">
                                      <div class="form-group">
                                          ...
                                          <select id="select-sales" name="sales" autocomplete="off" required >
                                              <option value="{{$detail->sales}}" selected>{{$detail->sales}}</option>
                                      ...
                                      <div class="form-group">
                                          ...
                                          <select id="select-barang" name="kios" autocomplete="off" required >
                                              <option value="{{$detail->kios}}" selected>{{$detail->kios}}</option>
                                              {{-- @foreach ($dataApi as $b) --}}
                                                  {{-- @if(is_array($b))
                                                      <option value="{{ $b['barang'] }}">{{ $b['barang'] }}</option>
                                                  @elseif(is_object($b))
